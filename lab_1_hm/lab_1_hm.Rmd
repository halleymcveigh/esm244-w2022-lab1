...
```{r}
sf_trees <- read_csv(here("data", "sf_trees", "sf_trees.csv")) # names() and summary() and head()
...
top_5_status <- sf_trees %>% 
  count(legal_status) %>% 
  drop_na(legal_status) %>% # can use ?drop_na() to see documentation of function
  rename(tree_count = n) %>% 
  relocate(tree_count) %>% 
...
shift-ctrl-c to comment/uncomment quickly
...
Read in the SF shapefile and add to the map
